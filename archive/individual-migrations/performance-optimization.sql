...
CREATE INDEX IF NOT EXISTS idx_card_clicks_created_at ON card_clicks(created_at);
...
GROUP BY fc.id, fc.user_id, fc.title, fc.slug, fc.description, fc.custom_fields, fc.is_active, fc.created_at, fc.updated_at;
...
BEGIN
  ...
  RAISE NOTICE 'Created indices for: cards, card_clicks, form_configs, form_submissions, profile_views, profiles, user_roles';
  RAISE NOTICE 'Created materialized views: cards_with_metrics, forms_with_metrics';
  RAISE NOTICE 'Created auto-refresh triggers for materialized views';
END $$;
