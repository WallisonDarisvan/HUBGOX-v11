-- Corrigir conflito de UNIQUE (profiles_username_key) ao aceitar convite
-- Estratégia: inserir novo profile com username temporário, migrar dados,
-- deletar o profile antigo e então restaurar o username original
...
BEGIN
  ...
  v_temp_username := v_username || '_tmp_' || left(user_id::text, 8);
  ...
  SET profile_id = accept_invitation.user_id
...
COMMENT ON FUNCTION public.accept_invitation(uuid, uuid) IS 'Aceita convite criando novo profile com username temporário para evitar conflito e depois restaura o username original.';