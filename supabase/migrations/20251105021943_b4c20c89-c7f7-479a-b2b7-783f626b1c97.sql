-- =====================================================
-- Adicionar created_by_admin_id para manter vínculo admin-perfil
-- =====================================================

-- 1. Adicionar coluna created_by_admin_id na tabela profiles
ALTER TABLE public.profiles 
ADD COLUMN created_by_admin_id uuid REFERENCES auth.users(id) ON DELETE SET NULL;

-- 2. Criar índice para performance
CREATE INDEX idx_profiles_created_by_admin_id 
ON public.profiles(created_by_admin_id);

-- 3. Migrar dados históricos: preencher created_by_admin_id 
-- para perfis que já foram vinculados via convites
UPDATE public.profiles p
SET created_by_admin_id = ui.invited_by_admin_id
FROM public.user_invitations ui
WHERE p.id = COALESCE(ui.linked_profile_id, ui.profile_id)
  AND ui.invited_by_admin_id IS NOT NULL
  AND p.created_by_admin_id IS NULL;

-- 4. Atualizar a função accept_invitation para preencher created_by_admin_id
...
BEGIN
    ...
    ELSE
        ...
        INSERT INTO public.profiles (
            ...
            is_activated, created_by_admin_id
        ) VALUES (
            ...
            v_footer_text_primary, v_footer_text_secondary, true, v_admin_id
        );
    END IF;
    ...
    IF NOT EXISTS (
        ...
        AND invited_by_admin_id = v_admin_id
        AND status = 'accepted'
    ) THEN
        RAISE EXCEPTION 'ERRO CRÍTICO: Vínculo admin-usuário não foi criado!';
    END IF;

    RAISE NOTICE '✓ Vínculo admin-usuário verificado: admin=% → user=%', v_admin_id, new_user_id;
    ...
    RAISE NOTICE 'Admin ID: %', v_admin_id;
    RAISE NOTICE 'created_by_admin_id: %', v_admin_id;
